...
function getAudioUrl(song: SongWithReason): string {
  ...
  // Fallback demo audio when an external API preview is unavailable.
...
function getInstagramReelsUrl(song: SongWithReason): string {
  ...
  const query = encodeURIComponent(
    `${song.title} ${song.artist} short video reel`,
  );
  return `https://www.youtube.com/results?search_query=${query}`;
}
...
export default function SongCard({
  ...
  function handlePlayPause() {
    ...
    if (isPlaying) {
      ...
      window.dispatchEvent(
        new CustomEvent("songPlaying", { detail: { songId: song.id } }),
      );

      // Update audio source based on play mode
      const audioUrl =
        playMode === "clip" ? getInstagramAudioUrl(song) : getAudioUrl(song);
  ...
  function togglePlayMode() {
    ...
    if (isPlaying && audioRef.current) {
      audioRef.current.pause();
      const newUrl =
        newMode === "clip" ? getInstagramAudioUrl(song) : getAudioUrl(song);
  ...
  return (
    <div
      className={`song-card ${isSelected ? "selected" : ""} ${liked ? "liked" : ""} ${isCurrentPlaying ? "playing" : ""}`}
    >
      ...
      <div className="card-labels">
        {song.label === "safe" && (
          <span className="label safe">
            <Star size={10} /> Safe Choice
          </span>
        )}
        {song.label === "unique" && (
          <span className="label unique">
            <Zap size={10} /> Unique Pick
          </span>
        )}
        {song.label === "trending" && (
